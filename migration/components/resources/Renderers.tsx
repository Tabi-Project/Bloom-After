...
export function renderBulletItem(text: string): string {
...
export function renderInfographic(resource: Resource): string {
  ...
  const data = resource.structured_content as InfographicContent;
...
export function renderMythBusting(resource: Resource): string {
  if (!resource?.structured_content) return `<p>Content unavailable.</p>`;
  const { myths, facts } = resource.structured_content as MythBustingContent;
...
export function renderMedia(resource: Resource): string {
  ...
  const { title, file_url, imageUrl, image_url, structured_content, media_format } = resource;
  const mediaImageUrl = imageUrl || image_url || "";
  const format = media_format || "audio";
  const paragraphs = (structured_content as MediaContent).summary_paragraphs || [];

  let playerHTML = "";

  if (!file_url) {
    playerHTML = `<div class="media-fallback"><p>${format} not available.</p></div>`;
