    ...
    return `
      <article class="clinic-card" data-id="${clinic.id}" tabindex="0">
        ...
        <div class="card-header">
          ...
          <p class="card-subtitle">${clinic.provider_type.replace('_', ' ')} • ${clinic.city}</p>
        </div>
        ${distanceHtml}
...
function openDetailsPanel(id) {
  ...
  DOM.panelContent.innerHTML = `
    ...
    <h3 class="panel-section-title">Services</h3>
    ...
    <div class="panel-reviews-section">
      ...
      <p class="panel-review-rating">★ ${clinic.rating} (${clinic.reviewCount} reviews)</p>
      
      <div class="panel-review-intro" id="review-intro-box">
        <p class="panel-review-intro-text">To ensure a safe environment, all reviews are vetted by our administrative team before appearing on Bloom After.</p>
        <button id="btn-reveal-form" class="btn-outline w-100">Write a Review</button>
      </div>

      <div class="review-form-container" id="review-form-container" hidden>
        <form id="review-form" class="review-form" novalidate>
          ...
          <div class="form-group">
            ...
            <div class="error-msg" id="text-error" hidden>Review must be at least 10 characters long.</div>
          </div>
          
          <button type="submit" id="btn-submit-review" class="btn-solid w-100">Submit for Moderation</button>
          
          <div class="review-success-state" id="review-success" hidden>
            <h4>Thank You!</h4>
            <p>Your review has been securely submitted to our moderation team. It will appear on the site once approved.</p>
          </div>
        </form>
      </div>
    </div>
