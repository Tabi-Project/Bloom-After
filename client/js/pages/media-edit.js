...
async function init() {
  ...
  document.getElementById('sidebar-root').innerHTML = renderAdminSidebar({ activePage: 'media-suggestions', totalPending: 0, currentRole: stored.isSuperAdmin ? 'superadmin' : 'moderator' });
  ...
  document.title = `${item.title || 'Media'} — Bloom Admin`;
  bindActions(item);
}
...
async function fetchItem(id) {
  try {
    ...
    return { id, title: 'Motherhood Unfiltered — Ep. 12', description: 'Podcast episode covering lived PPD experiences in West Africa. Very raw and relatable.', email: 'nkechi@example.com', link: 'https://spotify.com/motherhood', image_url: 'https://images.unsplash.com/photo-1478737270239-2f02b77fc618?w=400&q=70', mediaType: 'Podcast', status: 'pending', submittedAt: new Date().toISOString() };
  }
}
